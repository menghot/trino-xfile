...
public final class ParquetTypeUtils
{
    ...
    public static ColumnIO getArrayElementColumn(ColumnIO columnIO)
    {
        if(columnIO instanceof PrimitiveColumnIO ) {
            ...
            // The has logicalType annotation, it should be LIST or MAP
            if (groupColumnIO.getType().getLogicalTypeAnnotation() != null) {
                return groupColumnIO;
            }

            if (groupColumnIO.getChildrenCount()== 0) {
                throw new TrinoException(NOT_SUPPORTED, format("Parquet group name: %s has no children", groupColumnIO.getName()));
            }  else if (groupColumnIO.getChildrenCount() == 1) {
